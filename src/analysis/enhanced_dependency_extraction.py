...
class EnhancedDependencyExtractor:
    ...
    def _extract_maven_dependencies_enhanced(self, pom_file: Path) -> List[DependencyInfo]:
        ...
        try:
            ...
            for dependency in root.findall(".//maven:dependencyManagement//maven:dependency", namespace):
                dep_info = self._parse_maven_dependency(
                    dependency, namespace, properties, str(pom_file), 
                    scope="dependencyManagement", dependency_management=True
                )
                if dep_info:
                    dependencies.append(dep_info)
