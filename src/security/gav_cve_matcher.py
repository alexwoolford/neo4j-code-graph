    ...
    def matches_version(self, target_version: str) -> bool:
        ...
        try:
            ...
            if self.version_start_including:
                if target_ver < version.parse(self.version_start_including):
                    return False
            
            if self.version_start_excluding:
                if target_ver <= version.parse(self.version_start_excluding):
                    return False
            
            # Check end constraints
            if self.version_end_including:
                if target_ver > version.parse(self.version_end_including):
                    return False
            
            if self.version_end_excluding:
                if target_ver >= version.parse(self.version_end_excluding):
                    return False
...
class PreciseGAVMatcher:
    ...
    def _fuzzy_cpe_match(self, gav: GAVCoordinate, cve_data: Dict) -> Optional[float]:
        ...
        for cpe_uri, affected_product in cpe_matches:
            ...
            if (len(artifact_lower) > 4 and 
                artifact_lower in cpe_lower and 
                artifact_lower not in ['core', 'common', 'utils', 'base']):
                
                # Additional validation - check if group parts match
                group_matches = sum(1 for part in group_parts 
                                  if len(part) > 3 and part in cpe_lower)
    ...
    def validate_dependencies_against_cves(
        self, 
        dependencies: List[GAVCoordinate], 
        cve_list: List[Dict]
...
def run_validation_tests():
    ...
    for dep in test_deps:
        ...
        if confidence:
            logger.info(f"✅ MATCH: {dep.full_coordinate} -> CVE-2021-44228 (confidence: {confidence})")
        else:
