...
class RobustCVEManager:
    ...
    def fetch_targeted_cves(
        ...
        if partial_data:
            logger.info(
                f"🔄 Resuming: {len(partial_data)} CVEs cached, {len(remaining_terms)} terms remaining"
            )
        else:
            ...
            for i, (query_term, original_terms) in enumerate(search_queries):
                try:
                    ...
                    if (i + 1) % 5 == 0:
                        ...
                        logger.debug(
                            f"💾 Checkpoint: {len(all_cves)} CVEs, {len(completed_terms_set)} terms completed"
                        )
                    ...
                    self._save_partial_targeted_cache(cache_key, all_cves, completed_terms_set)
                    logger.info(
                        f"💾 Saved {len(all_cves)} CVEs, {len(completed_terms_set)} terms completed"
                    )
                    raise
