...
class RobustCVEManager:
    ...
    def fetch_targeted_cves(self, api_key: Optional[str], search_terms: Set[str],
        ...
        max_requests_per_window = self.requests_per_30s_with_key if api_key else self.requests_per_30s_no_key

        logger.info(f"🎯 **TARGETED CVE SEARCH** (not downloading entire database!)")
        ...
        if partial_data:
            ...
            logger.info(f"🌐 Starting fresh targeted search")
            partial_data = []
        ...
        with tqdm(desc="Searching dependencies", total=len(search_queries), unit=" queries") as pbar:
    ...
    def _is_relevant_to_terms(self, cve: Dict, terms: Set[str]) -> bool:
        ...
        for term in terms:
            ...
            if '.' in term and 'jetbrains' in term_lower:
                ...
                elif any(ide_tool in cve_text for ide_tool in [
                    'teamcity', 'intellij', 'idea', 'youtrack', 'space',
                    'hub', 'kotlin', 'resharper', 'rider', 'webstorm',
                    'phpstorm', 'pycharm', 'rubymine', 'clion', 'goland'
                ]):
                    return False
                # If it's just generic "jetbrains" mention without library context, skip it
                elif 'jetbrains' in cve_text and not any(lib_term in cve_text for lib_term in [
                    'annotation', 'library', 'maven', 'gradle', 'jar'
                ]):
                    return False
