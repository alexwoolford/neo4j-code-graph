...
- *`(Method)-[:CALLS_EXTERNAL]->(Import)`*: Frontier call into an external library API; edge carries `confidence` (`HIGH`/`MEDIUM`/`LOW`), `confidence_rank`, `target_class`, `call_type`, and `call_count`. Resolution uses declared imports and receiver declared types only (reflection/DI invisible).
...
- `calls_community` (Long): Community id from Louvain over the CALLS graph.
