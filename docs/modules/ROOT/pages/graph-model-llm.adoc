...
- `(Method)-[:CALLS_EXTERNAL {confidence}]->(Import)` — derived method→Import frontier edge into external library APIs with `confidence` HIGH/MEDIUM/LOW from receiver-declared-type resolution; reflection/DI calls are not captured.
...
----
{
  ...
  "relationships": [
    ...
    {"type": "CALLS_EXTERNAL", "from": "Method", "to": "Import", "properties": ["confidence", "confidence_rank", "call_count"]}
  ],
  "canonical_chains": [
    ["Commit", "CHANGED", "FileVer", "OF_FILE", "File"],
    ["File", "DECLARES", "Method"],
