...
TARGET_DOC = Path("docs/modules/ROOT/pages/graph-model.adoc")
...
def _collect_properties(driver: Driver, database: str | None) -> Mapping[str, List[Tuple[str, Tuple[str, ...]]]]:
...
def main() -> None:
    ...
    try:
        ...
        logger.warning("Neo4j connection unavailable; generating docs from known schema only: %s", e)
