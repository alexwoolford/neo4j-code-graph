...
from analysis.code_analysis import (
    extract_file_data, compute_embeddings_bulk, load_model_and_tokenizer,
...
def get_files_needing_embeddings(session, repo_root):
    ...
    logger.info(f"Found {len(files_needing_processing)} files needing embeddings out of {len(java_files)} total")
...
def process_remaining_files(session, files_to_process, repo_root, dependency_versions):
    ...
    logger.info(f"Computing embeddings for {len(file_snippets)} files and {len(method_snippets)} methods...")
...
def main():
    ...
    parser.add_argument("--files-only", action="store_true", help="Only process missing file embeddings")
    ...
    try:
        with driver.session(database=config[3]) as session:
            ...
            # Check current state
            result = session.run("MATCH (f:File) RETURN count(f) as total, count(f.embedding) as with_embeddings")
            stats = result.single()
            logger.info(f"Current state: {stats['with_embeddings']}/{stats['total']} files have embeddings")
            ...
            if not args.files_only:
                ...
                if calls_count > 0:
                    ...
                    logger.info("   Use --files-only flag if you only want to complete file embeddings")
                else:
