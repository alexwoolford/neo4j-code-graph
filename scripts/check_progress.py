...
def check_database_state(driver, database):
    ...
    with driver.session(database=database) as session:
        ...
        result = session.run("MATCH (f:File) WHERE f.embedding IS NOT NULL RETURN count(f) as count")
        ...
        result = session.run("MATCH (m:Method) WHERE m.embedding IS NOT NULL RETURN count(m) as count")
