...
import pytest
from security.gav_cve_matcher import (
    GAVCoordinate, 
    PreciseGAVMatcher, 
    AffectedProduct,
    CVEVulnerability
...
class TestVersionMatching:
    ...
    def test_version_within_vulnerable_range(self):
        ...
        assert affected.matches_version("2.14.1") == True
        assert affected.matches_version("2.10.0") == True
        assert affected.matches_version("2.0.0") == True
        
        # Should NOT match fixed versions
        assert affected.matches_version("2.15.0") == False
        assert affected.matches_version("2.17.0") == False
        assert affected.matches_version("1.9.0") == False
    ...
    def test_version_excluding_constraints(self):
        ...
        assert affected.matches_version("5.0.1") == True
        assert affected.matches_version("5.3.21") == True
        assert affected.matches_version("5.2.0") == True
        
        # Should NOT match boundary versions
        assert affected.matches_version("5.0.0") == False
        assert affected.matches_version("5.3.22") == False
        assert affected.matches_version("4.9.0") == False
    ...
    def spring_cve_data(self):
        ...
        return {
            ...
            "descriptions": [
                {
                    "lang": "en", 
                    "value": "Spring Framework RCE via Data Binding on JDK 9+..."
                }
            ],
            "configurations": [
                {
