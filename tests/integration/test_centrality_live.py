...
def test_hits_analysis_live():
    ...
    from src.analysis.centrality import create_call_graph_projection, run_hits_analysis
    from src.data.schema_management import setup_complete_schema
    ...
    try:
        G = create_call_graph_projection(gds)
        auth, hubs = run_hits_analysis(gds, G, top_n=3, write_back=False)
        if auth is None and hubs is None:
            pytest.skip("HITS not available in this GDS version")
        assert auth is not None and hubs is not None
    finally:
        try:
            gds.graph.drop("method_call_graph")
        except Exception:
